...
(function () {
  ...
  const Utils = {
    ...
    getTipoObra() {
      ...
      const nivelBib = document.getElementById("id_nivel_bibliografico")?.value;
  ...
  const Serializer = {
    ...
    serialize(form) {
      const data = {
        ...
        meta: {
          ...
          pestana: typeof currentTabIndex !== "undefined" ? currentTabIndex : 0,
        },
      };
      ...
      form
        ...
        .forEach((container) => {
          ...
          data.formsets[prefix] = this._serializeFormset(container, prefix);
        });
    ...
    _serializeSubcamposGlobales(form, data) {
      ...
      form
        .querySelectorAll("[data-borrador-container]")
        .forEach((container) => {
          ...
          container
            ...
            .forEach((subRow) => {
              ...
              const input = subRow.querySelector("input, select, textarea");
              if (input) {
    ...
    _serializeCamposSimples(form, data) {
      ...
      for (const [name, value] of formData.entries()) {
        // IMPORTANTE: Nunca guardar el token CSRF - siempre debe usarse el actual
        ...
        if (name.includes("-TOTAL_FORMS") || name.includes("-INITIAL_FORMS"))
          continue;
        if (name.includes("-MIN_NUM_FORMS") || name.includes("-MAX_NUM_FORMS"))
          continue;
        ...
        row.querySelectorAll("input, select, textarea").forEach((input) => {
          ...
          if (match) {
            ...
            if (field !== "DELETE" && field !== "id") {
              rowData.campos[field] = this._getInputValue(input);
            }
          }
        });
        ...
        if (input?.tagName === "SELECT") {
          ...
          if (firstOption) {
            if (!firstOption.value || firstOption.value === "") {
              ...
              const selectedDefault = input.querySelector("option[selected]");
  ...
  const Restorer = {
    ...
    _normalize(data) {
      ...
      const oldSubcampos = data._subcampos_dinamicos || data.subcampos || {};
    ...
    _restoreCamposSimples(form, campos) {
      for (const [name, value] of Object.entries(campos)) {
        // IMPORTANTE: Nunca restaurar el token CSRF - debe usarse el de la página actual
        if (name === "csrfmiddlewaretoken") continue;
        ...
        if (Array.isArray(value)) {
          ...
          inputs.forEach((input) => this._setInputValue(input, value));
        }
      }
    },
    ...
    async _restoreFormsets(form, formsets) {
      for (const [prefix, rows] of Object.entries(formsets)) {
        ...
        let currentCount = Array.from(existingRows).filter(
          (r) => r.style.display !== "none" && !r.classList.contains("d-none")
        ).length;
        ...
        for (let i = 0; i < rows.length && i < allRows.length; i++) {
          await this._restoreFormsetRow(allRows[i], prefix, i, rows[i]);
        }
      }
    },
    ...
    async _restoreFormsetRow(row, prefix, index, rowData) {
      ...
      for (const [field, value] of Object.entries(rowData.campos)) {
        ...
        let input = row.querySelector(`[name="${prefix}-${index}-${field}"]`);
      ...
      for (const [tipo, values] of Object.entries(rowData.subcampos || {})) {
    ...
    async _restoreSubcampos(row, tipo, parentIndex, values) {
      ...
      const addButton = row.querySelector(`[data-borrador-add="${tipo}"]`);

      // Obtener filas de subcampos existentes
      let subRows = container.querySelectorAll(".subcampo-row:not(.d-none)");

      // Llenar valores
      for (let i = 0; i < values.length; i++) {
        if (i < subRows.length) {
          // Usar fila existente
          const input = subRows[i].querySelector("input, select, textarea");
          if (input) {
          ...
          // Re-obtener filas
          subRows = container.querySelectorAll(".subcampo-row:not(.d-none)");
    ...
    async _restoreSubcamposGlobales(form, subcamposGlobales) {
      for (const [tipo, values] of Object.entries(subcamposGlobales)) {
        ...
        if (!addButton) {
          ...
          if (parent) {
            addButton = parent.querySelector(`[data-borrador-add="${tipo}"]`);
          }
        }
    ...
    async _rehydrateObraLinks(form) {
      ...
      for (const hidden of hiddenInputs) {
        ...
        if (!cache.has(obraId)) {
          try {
            const resp = await fetch(`${URLS.searchObras}?id=${obraId}`);
            const data = await resp.json();
            cache.set(obraId, data?.results?.[0]?.num_control || null);
          } catch {
  ...
  const API = {
    async save(data, isAuto = false) {
      try {
        ...
        const response = await fetch(isAuto ? URLS.autoSave : URLS.save, {
          method: "POST",
          headers: {
