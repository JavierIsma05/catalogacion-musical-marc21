...
function logearDatosFormulario(form) {
  ...
  mostrarBloque("BLOQUE 0XX - CAMPOS DE CONTROL", datosOrganizados.bloque_0xx);
  mostrarBloque(
  ...
  mostrarBloque("BLOQUE 3XX - DESCRIPCIÓN FÍSICA", datosOrganizados.bloque_3xx);
    ...
    if (datos.length > 0) {
      ...
      datos.forEach((item, idx) => {
        if (item.indice !== undefined) {
          console.log(
            `   [${parseInt(item.indice) + 1}] ${item.campo || ""}: "${
              item.valor
            }"`
          );
        } else {
          console.log(`   [${idx + 1}] ${item.campo}: "${item.valor}"`);
        }
      });
    }
  }
...
function mostrarEstadisticas() {
  ...
  stats.campos0xx.forEach((campo) => {
    if (campo.count >= 0) {
      console.log(`   ${campo.nombre}: ${campo.count} campo(s) agregado(s)`);
    }
  });
...
window.verEstadoFormulario = function () {
  ...
  // Inicializar el canvas para el primer incipit (index 0), si existe

  if (typeof window.initIncipit031 === "function") {
    window.initIncipit031({ index: 0 });
  }
};
