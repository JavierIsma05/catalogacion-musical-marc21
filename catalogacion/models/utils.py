...
def generar_numero_control(tipo_registro):
    ...
    with transaction.atomic():
        ...
        secuencia, created = (
            NumeroControlSecuencia.objects.select_for_update().get_or_create(
                tipo_registro=tipo_registro, defaults={"ultimo_numero": 0}
            )
        )
...
def obtener_pais_principal(obra):
    ...
    try:
        ...
        print(
            f"🔍 DEBUG: Países encontrados para obra {obra.num_control}: {[p.codigo_pais for p in paises]}"
        )
...
def validar_obra_coleccion(obra):
    ...
    if hasattr(obra, "contenidos_505"):
        if not obra.contenidos_505.exists():
            errores["contenidos_505"] = (
                "Las colecciones deben registrar al menos un contenido 505."
            )

    # Verificar si tiene enlaces 774
    if hasattr(obra, "enlaces_unidades_774"):
        if not obra.enlaces_unidades_774.exists():
            errores["enlaces_unidades_774"] = (
                "Las colecciones deben detallar obras contenidas (774)."
            )
    ...
    if hasattr(obra, "enlaces_documento_fuente_773"):
        if obra.enlaces_documento_fuente_773.exists():
            errores["enlaces_documento_fuente_773"] = (
                "Las colecciones no pueden enlazarse mediante 773."
            )
    ...
    if obra.tipo_registro == "d":
        if obra.isbn or obra.ismn or obra.numero_editor:
            errores["isbn"] = (
                "Las colecciones manuscritas no utilizan identificadores 020/024/028."
            )
...
def validar_obra_en_coleccion(obra):
    ...
    if hasattr(obra, "enlaces_documento_fuente_773"):
        if not obra.enlaces_documento_fuente_773.exists():
            errores["enlaces_documento_fuente_773"] = (
                "Debe registrar al menos un enlace 773 a la colección contenedora."
            )
    ...
    if hasattr(obra, "enlaces_unidades_774"):
        if obra.enlaces_unidades_774.exists():
            errores["enlaces_unidades_774"] = (
                "Las obras individuales no deben registrar 774."
            )
...
def validar_obra_independiente(obra):
    ...
    if hasattr(obra, "enlaces_documento_fuente_773"):
        if obra.enlaces_documento_fuente_773.exists():
            errores["enlaces_documento_fuente_773"] = (
                "Las obras independientes no utilizan 773."
            )
    ...
    if hasattr(obra, "enlaces_unidades_774"):
        if obra.enlaces_unidades_774.exists():
            errores["enlaces_unidades_774"] = (
                "Las obras independientes no utilizan 774."
            )
    ...
    if obra.tipo_registro == "c":
        if hasattr(obra, "contenidos_505"):
            if obra.contenidos_505.exists():
                errores["contenidos_505"] = (
                    "Las obras individuales impresas no utilizan 505."
                )
