...
class CrearObraView(CreateView):
    ...
    def _save_numeros_obra_787(self, formset):
        """
        Procesar inputs de números de obra generados por JavaScript.
        Los inputs tienen nombres como: numero_enlace_787_0_1234567890
        donde 0 es el índice del enlace y 1234567890 es el timestamp.
        """
        from catalogacion.models import NumeroObraRelacionada787
        
        # Agrupar números por índice de enlace
        numeros_por_enlace = {}
        
        for key, value in self.request.POST.items():
            if key.startswith('numero_enlace_787_') and value.strip():
                try:
                    # Extraer índice del enlace: numero_enlace_787_0_1234567890 -> 0
                    parts = key.split('_')
                    enlace_index = int(parts[3])
        ...
        for index, form in enumerate(formset):
            if form.instance.pk and index in numeros_por_enlace:
                ...
                # Crear nuevos números
                for numero in numeros_por_enlace[index]:
                    NumeroObraRelacionada787.objects.create(
